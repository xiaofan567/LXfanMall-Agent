    ...
    public String notify(Map<String, String> params) {
        ...
        if (signVerified) {
            ...
            if("TRADE_SUCCESS".equals(tradeStatus)){
                ...
                // 支付回调幂等性：检查该订单是否已处理过
                String payNotifyKey = "mall:pay:notify:" + outTradeNo;
                if (redisService.hasKey(payNotifyKey)) {
                    log.info("支付回调重复通知，订单号:{}，直接返回success", outTradeNo);
                    return "success";
                }
                redisService.set(payNotifyKey, "1", 86400); // 24小时内去重
